...
class CategorySuggestionAddForm extends AbstractForm {
    ...
    public $neededPermissions = array (
            'user.wiki.category.write.canSuggestCategories'
    );
    ...
    public function readFormParameters() {
        ...
        if(isset($_POST['parentCategory']))
            $this->parentCategoryID = intval($_POST['parentCategory']);
        if(I18nHandler::getInstance()->isPlainValue('title'))
            $this->title = I18nHandler::getInstance()->getValue('title');
        if(I18nHandler::getInstance()->isPlainValue('reason'))
            $this->reason = I18nHandler::getInstance()->getValue('reason');
    }
