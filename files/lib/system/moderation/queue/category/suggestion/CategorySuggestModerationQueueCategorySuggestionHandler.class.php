...
/**
 * An implementation of IModerationQueueCategorySuggestionHandler for category
 * suggestions.
 *
...
class CategorySuggestModerationQueueCategorySuggestionHandler extends AbstractModerationQueueHandler implements IModerationQueueCategorySuggestionHandler {
	...
	public function acceptSuggestion(ModerationQueue $queue) {
		if($this->isValid($queue->objectID)) {
			$categorySuggestionAction = new CategorySuggestionAction(array (
					$this->getCategorySuggestion($queue->objectID) 
			), 'accept');
	...
	/**
	 * Returns a categorySuggestion object by suggestion id or null if
	 * suggestion id is invalid.
	 *
	...
	public function populate(array $queues) {
		...
		$categorySuggestionList->getConditionBuilder()->add("category_suggestion.suggestionID IN (?)", array (
				$objectIDs 
		));
