...
<head>
<title>{lang}wiki.article.watchedArticles{/lang} {if $pageNo >
	1}- {lang}wcf.page.pageNo{/lang} {/if}- {PAGE_TITLE|language}</title> {include
file='headInclude'}
...
<body id="tpl{$templateName|ucfirst}">
	...
	<header class="boxHeadline">
		<h1>
			{lang}wiki.article.watchedArticles{/lang} <span class="badge">{#$items}</span>
		</h1>
	</header>
	...
	<div class="contentNavigation">
		{pages print=true assign=pagesLinks application='wiki'
		controller='WatchedArticleList'
		link="pageNo=%d&sortField=$sortField&sortOrder=$sortOrder"}

		{hascontent}
		<nav>
			<ul>{content} {event name='contentNavigationButtonsTop'}
				{/content}
			</ul>
		</nav>
		{/hascontent}
	...
	{if $objects|count}
	<div
		class="marginTop tabularBox tabularBoxTitle messageGroupList wikiArticleList jsClipboardContainer"
				...
				<tr class="article{if $article->isNew()} new{/if}"
					...
					<td class="columnText columnSubject">
						...
						<h1>
							{hascontent}
							<ul class="labelList">
								{content} {foreach from=$article->getAssignedLabels()
								item=label}
								<li><a
							...
							{/hascontent} <a
								...
								data-article-id="{@$article->articleID}">{$article->getTitle()}</a>
						</h1> <small> <a
					...
					<td class="columnText columnCategory">
						<p>
							<a
								href="{link controller='Category' application='wiki' object=$article->getCategory()}{/link}">{$article->getCategory()->getTitle()}</a>
						</p>
					</td>
					<td class="columnDigits columnComments"><p>
							<a
								href="{link controller='Article' application='wiki' categoryName=$article->getCategory()->getTitle() object=$article->getActiveVersion()}#discuss{/link}">{#$article->getCommentList()|count}</a>
						</p></td>
	...
	<div class="contentNavigation">
		{@$pagesLinks} {hascontent}
		<nav>
			<ul>{content} {event name='contentNavigationButtonsBottom'}
				{/content}
			</ul>
		</nav>
		{/hascontent}
