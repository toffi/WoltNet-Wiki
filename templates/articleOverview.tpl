					...
					<footer class="messageOptions contentOptions marginTop clearfix">
						<nav>
							<ul class="smallButtons">
								...
								{if !$article->isActive && $article->getModeratorPermission('canActivateArticle')}<li><a href="{link application='wiki' controller='ArticleActivate' object=$article}{/link}" class="button"><span class="icon icon16 icon-ok"></span> <span>{lang}wiki.global.button.activate{/lang}</span></a></li>{/if}
								{if $article->isEditable()}<li><a href="{link application='wiki' controller='ArticleEdit' object=$article}{/link}" class="button"><span class="icon icon16 icon-pencil"></span> <span>{lang}wiki.global.button.edit{/lang}</span></a></li>{/if}
								{if $article->isTrashable()}<li><a href="{link application='wiki' controller='ArticleTrash' object=$article}{/link}" class="button"><span class="icon icon16 icon-trash"></span> <span>{lang}wiki.global.button.trash{/lang}</span></a></li>{/if}
							</ul>
						</nav>
					</footer>
				</div>
