					...
					<footer class="messageOptions contentOptions marginTop clearfix">
						<nav>
							<ul class="smallButtons buttonGroup">
								...
								{if $article->isEditable()}<li><a href="{link application='wiki' controller='ArticleEdit' object=$article}{/link}" class="button jsTooltip" title="{lang}wiki.global.button.edit{/lang}"><span class="icon icon16 icon-pencil"></span></a></li>{/if}
								{if $article->isTrashable()}<li><a href="{link application='wiki' controller='ArticleTrash' object=$article}{/link}" class="button jsTooltip" title="{lang}wiki.global.button.trash{/lang}"><span class="icon icon16 icon-trash"></span></a></li>{/if}
							</ul>
						</nav>
					</footer>
				</div>
