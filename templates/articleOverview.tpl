                    ...
                    <footer class="messageOptions contentOptions marginTop clearfix">
                        <nav>
                            <ul class="smallButtons buttonGroup">
                                ...
                                {if !$article->isActive && $article->getModeratorPermission('canActivateArticle')}<li><a href="{link application='wiki' controller='ArticleVersion' action='activate' object=$article versionID=$article->versionID}{/link}" class="button jsTooltip" title="{lang}wiki.global.button.activate{/lang}"><span class="icon icon16 icon-ok"></span></a></li>{/if}
                                ...
                                {if $article->isTrashable()}<li><a href="{link application='wiki' controller='ArticleVersion' action='trash' object=$article}{/link}" class="button jsTooltip" title="{lang}wiki.global.button.trash{/lang}"><span class="icon icon16 icon-trash"></span><span>{lang}wiki.global.button.trash{/lang}</span></a></li>{/if}
                            </ul>
                        </nav>
                    </footer>
                </div>
