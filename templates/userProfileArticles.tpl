          ...
          {foreach from=$userArticleList item=articleList}
            <li class="wikiProfileContainer box48 marginTop">
              ...
              <hgroup class="wikiProfileContainerHeadline">
                ...
                <h2><small>{if $articleList->userID}<a href="{link controller='User' object=$articleList->getUserProfile()->getDecoratedObject()}{/link}">{$articleList->username}</a>{else}{$articleList->username}{/if} - {@$articleList->time|time} - {lang}wcf.user.profile.userArticles.category{/lang} <a href="{link application='wiki' controller='Category' object=$articleList->getCategory()}{/link}" class="jsTooltip" title="{$articleList->getCategory()->getTitle()}">{$articleList->getCategory()->getTitle()}</a></small></h2>
              </hgroup>
            </li>
          {/foreach}
        {/content}
