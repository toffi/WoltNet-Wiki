			...
			{foreach from=$updatedArticleList item=updatedArticleObject}
				<li class="sidebarBox box24">
					...
					<div class="sidebarBoxHeadline">
						<h1><a href="{link controller='Article' object=$updatedArticleObject->getActiveVersion() categoryName=$updatedArticleObject->getCategory()->getTitle()}{/link}" class="wikiArticleTopicLink articleLink" data-article-id="{@$updatedArticleObject->articleID}" data-sort-order="DESC" title="{$updatedArticleObject->getTitle()}">{$updatedArticleObject->getTitle()}</a></h1>
