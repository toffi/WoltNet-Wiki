...
<fieldset id="wikiLabelManagementForm">
	<legend>{lang}wiki.article.label.management.addLabel{/lang}</legend>
	
	<dl>
		<dt><label for="labelName">{lang}wiki.article.label.labelName{/lang}</label></dt>
			...
			<ul id="labelManagementList">
				{foreach from=$cssClassNames item=cssClassName}
					<li><label>
						...
						<span class="badge label{if $cssClassName != 'none'} {@$cssClassName}{/if}">{lang}wiki.article.label.placeholder{/lang}</span>
					</label></li>
				{/foreach}
			</ul>
		</dd>
